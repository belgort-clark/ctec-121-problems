...
def test_single_items():
    ...
    items = ["apple", "banana"]
    output = "1 APPLE\n1 BANANA"
    check50.run("python3 grocery.py").stdin(items[0], prompt=False).stdin(
        items[1], prompt=False
    ).stdin(EOF, prompt=False).stdout(regex(output), output, regex=True).exit()
...
def test_single_and_multiple_items():
    ...
    check50.run("python3 grocery.py").stdin(items[0], prompt=False).stdin(
        items[1], prompt=False
    ).stdin(items[2], prompt=False).stdin(EOF, prompt=False).stdout(
