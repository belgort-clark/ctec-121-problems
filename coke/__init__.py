...
def test_25():
    ...
    input = "25"
    output = "25"
    check50.run("python3 coke.py").stdin(input, prompt=True).stdout(
...
def test_multiple():
    ...
    input = "10"
    output = "30"
    check50.run("python3 coke.py").stdin(input, prompt=True).stdin(
        input, prompt=True
    ).stdout(regex(output), output, regex=True).kill()
...
def test_terminate():
    ...
    check50.run("python3 coke.py").stdin(input, prompt=True).stdin(
        input, prompt=True
    ).stdin(input, prompt=True).stdin(input, prompt=True).stdin(
        input, prompt=True
    ).stdout(
        regex(output), output, regex=True
    ).exit()
...
def test_change():
    ...
    check50.run("python3 coke.py").stdin(input, prompt=True).stdin(
        "10", prompt=True
    ).stdin(input, prompt=True).stdout(regex(output), output, regex=True).exit()
